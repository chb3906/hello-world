<template>
  ...
  <div class="wrap">
    ...
    <div class="content">
      ...
      <ul class="btns_ul">
        <li id="lable1" :class="lableId==1?'btns_li_active':''" @click="getListById('1')">理赔速度快(<span>0</span>)</li>
        <li id="lable2" :class="lableId==2?'btns_li_active':''" @click="getListById('2')">维修方便(<span>0</span>)</li>
        <li id="lable3" :class="lableId==3?'btns_li_active':''" @click="getListById('3')">科技感强(<span>0</span>)</li>
        <li id="lable4" :class="lableId==4?'btns_li_active':''" @click="getListById('4')">服务态度好(<span>0</span>)</li>
        <li id="lable5" :class="lableId==5?'btns_li_active':''" @click="getListById('5')">赔付金额准确(<span>0</span>)</li>
      ...
      <ul class="evaluate_ul" v-for='(item, index) in evaluateList'>
        ...
        <li class="btns_li" v-if="item.lable">
          <span v-if="item.lable&&(','+item.lable+',').indexOf(',1,')!=-1">理赔速度快</span>
          <span v-if="item.lable&&(','+item.lable+',').indexOf(',2,')!=-1">维修方便</span>
          <span v-if="item.lable&&(','+item.lable+',').indexOf(',3,')!=-1">科技感强</span>
          <span v-if="item.lable&&(','+item.lable+',').indexOf(',4,')!=-1">服务好态度好</span>
          <span v-if="item.lable&&(','+item.lable+',').indexOf(',5,')!=-1">赔付金额准确</span>
        </li>
...
<script>
  ...
  import Jquery from "jquery";
    ...
    created() {
		  //修改页面标题
      Jquery(document).find('title').text("理赔评价");
      this.pageCount = 1;
      this.lableId = '';
      this.pageIndex = 1;
      this.initFlag = true;
    ...
    methods: {
      ...
      getList(){
        this.query = false;
        this.$http.post('/sunmc/claimsEvaluation/claimsEvaluationListByApp', {
          "prdId"   :this.prdId,
          ...
          if(res.status == 200){
            ...
            this.evaluateList = data.claimsEvaluateList;
          }
        }).catch((err) => {
          this.queryFlag = true;
          console.log(err);
