            ...
            <div class="tbxx">
              ...
              <table class="tbxx_table">
                ...
                <tr>
                  ...
                  <td>
                    <el-date-picker class="inputP" :default-value="startDefaultValue" v-model="startDate" type="date" placeholder="选择起保日期时间" :pickerOptions='pickerOptions' :prefix-icon='icon' value-format="yyyy-MM-dd" @change='changeStartTime'></el-date-picker>
                    零时起 至
              ...
              <div class="prdStyles-wrap">
                <!-- <div class="name">
                  <div></div>
                  <div>保障类型</div>
                  <div class="type" v-for="(prdDuty,index) in specificationData.prdStyleSpecification[0].prdDutys" :key="index + 'prdDuty'" :title="prdDuty.dutyType ">{{ (index == 0 ||(index != 0 && prdDuty.dutyType != (specificationData.prdStyleSpecification[0].prdDutys)[index-1].dutyType)) ? prdDuty.dutyType : '' }}</div>
                </div> -->
                <div class="name">
                  <div></div>
                  <div>保障范围</div>
                  <div v-for="(prdDuty, index) in specificationData.prdStyleSpecification[0].prdDutys" :title="prdDuty.name + '\n' + prdDuty.value " :key="index + 'bzfw'">
                ...
                <div class="remark" :class="{on:i.prdItemId == specificationData.prdItemId}" v-for="(i,indexA) of specificationData.prdStyleSpecification" :key="indexA" @click="handletype(i)">
                  <div>{{ i.styleName }}<img v-show="i.prdItemId == specificationData.prdItemId" style="position: absolute;top: 0;right: 0;" src="../../assets/proToubaoxinxiPC/jbdh.png" /></div>
                  <div>保额</div>
                  <div v-for="(prdDuty, indexB) in i.prdDutys" :key="indexB + 'prdDutys'" v-if="prdDuty.name != '扩展24小时意外(可加购)'">{{ prdDuty.remark }}</div>
                  <!-- <div class="age-wrap" v-for="(item, indexAge) in typeData" :key="indexAge + 'age'" v-if="item.name == '年龄'" v-show="i.prdItemId == specificationData.prdItemId">
                  ...
                  <div class="age-wrap" v-for="(item, indexAge) in typeData" :key="indexAge + '元'" v-if="item.name == '年龄'" v-show="i.prdItemId == specificationData.prdItemId">
                    <span v-for="(haha,index) in item.entries" :key="index" style="color: #999999;"><span style="color: #FF8A00;font-weight: 700;">{{specificationData.price}}</span>元/人</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <tr v-for="(i) in info.items" :key='i.itemName'>
                  ...
                  <td>
                    ...
                    <div v-if="i.itemCode == 1">
                      <input type="text" class="tbxx_input" v-model="i.userName" placeholder="请输入姓名" @blur="handleCommonCheck(i,info.pageType)" @click="onSensors('PC-填单页-填写投保人姓名', $event)" disabled>
                      <span class="redStar">*</span>
                    </div>
                    <!--证件类型-->
                    <select v-else-if="i.isHasOption && i.itemCode == 3" :key="index+'Selected'" class="select-box" v-model="i.userName" @change="onSensors('PC-填单页-选择投保人证件类型', $event)" disabled>
                      <option v-for="itemy in i.options" :value="itemy.id" :key="itemy.id">{{itemy.name}}</option>
                    </select>
                    <!--证件编号-->
                    <div v-else-if="i.itemCode == 4">
                      <input type="text" class="tbxx_input" placeholder="请填写证件号码" v-model="i.userName" @blur="handleCommonCheck(i,info.pageType)" @change="handleCardTypeForGroup(i.itemCode,i.userName)" @click="onSensors('PC-填单页-填写投保人证件号码', $event)" disabled>
                      <span class="redStar">*</span>
                    </div>
                    <!--行业类型-->
                    ...
                    <div v-else-if="i.itemCode == 6">
                      <input type="text" class="tbxx_input" placeholder="请填写联系电话" v-model="i.userName" @blur="handleCommonCheck(i,info.pageType)" maxlength="16" @click="onSensors('PC-填单页-填写投保人联系电话', $event)" disabled>
                      <span class="redStar">*</span>
                    </div>
                    <!--邮箱-->
                    <div v-else-if="i.itemCode == 7">
                      <input type="text" class="tbxx_input" placeholder="请填写邮箱" v-model="i.userName" @blur="handleCommonCheck(i,info.pageType)" @click="onSensors('PC-填单页-填写邮箱', $event)" disabled>
                      <span class="redStar">*</span>
                    </div>
                    <!--所在地区-->
                    ...
                    <div v-else-if="i.itemCode == 15">
                      <input type="text" class="tbxx_input" placeholder="请填写详细地址" v-model="i.userName" @blur="handleCommonCheck(i,info.pageType)" @click="onSensors('PC-填单页-选择详细地址', $event)" disabled>
                      <span class="redStar">*</span>
                    </div>
                    <!--营业执照-->
            ...
            <div class="tbxx" v-for="(info,index) in page.types" :key="index" v-if="info.pageType == 5">
              <div class="title">
                ...
                <div class="addInfo-wrap">
                  ...
                  <!-- <span v-show="pagedata.minEmployeesNum > 1 && pagedata.maxEmployeesNum > 1"><a href="../../../static/雇员信息录入Excel模板.xlsx" style="color: #fff;">下载模板<img src="../../assets/proToubaoxinxiPC/an-xz.png" alt=""> </a></span> -->
                  <!--批量上传信息-->
                  <!-- <input-excel @getResult="getMyExcelData" v-show="pagedata.minEmployeesNum > 1 && pagedata.maxEmployeesNum > 1"></input-excel> -->
                </div>
              </div>
              <div>
      ...
      <div class="calculate_info">
        <div class="calculate_wrap">
          ...
          <!-- <div class="age-wrap" v-for="(item, indexAge) in typeData" :key="indexAge + 'age'" v-if="item.name == '年龄'">
            <span v-for="(haha,index) in item.entries" :key="index">{{ haha.value }}<span style="font-weight: 700;">{{specificationData.price}}</span>元/人</span>
          </div> -->
            ...
            <div class="tbxx">
              ...
              <div class="tb_table">
                ...
                <div class="tb_table_item">
                  ...
                  <div class="content">{{startDate}} 零时起 至 {{endDate}} 二十四时 止</div>
                </div>
              </div>
            </div>
            <!--方案介绍信息-->
            <div class="prd-introduce" v-for="(item,index) in specificationData.prdStyleSpecification" v-if="item.prdItemId == currentItemId">
              <div class="title">
                <!-- <span>保障类型</!-->
                <span>保障范围</span>
                <span>保额/人</span>
              ...
              <div class="content" v-for="(prd,i) in item.prdDutys" v-if="prd.name != '扩展24小时意外(可加购)'">
                <!-- <span :title="prd.dutyType">{{ (i == 0 ||(i != 0 && prd.dutyType != (specificationData.prdStyleSpecification[index].prdDutys)[i-1].dutyType)) ? prd.dutyType : '' }}</span> -->
...
export default {
  ...
  data() {
    return {
      ...
      prdStyles: JSON.parse(window.sessionStorage.getItem('prdStyles')),
      isCache: false,
      createPolicySign: '',
      insuredInfos: [],
      userSign: '',// 用户标识
      ...
      prdId: this.$route.query.id, // 产品id
      ...
      companyId: this.$route.query.companyId,// 公司标识码
      ...
      specificationData: {}, // 确认投保数据
      currentItemId: '', // 当前方案明细id
      currentStyleId: '',// 当前方案id
      typeData: [], // 当前方案的数据明细prdSpecificationCalcs
      entries: [],// 年龄的明细数组
      upParams: {},
  ...
  methods: {
    ...
    getCityData() {
      this.$http.get('admin/sunEcDcompany/getPcCityJson').then(res => {
        this.citySelectOptions = res.data.content
    ...
    isAgeInArea(IDNo) {
      let age = GetAge(IDNo, this.startDate, 1)
      let day = GetDay(IDNo, this.startDate, 1)
    ...
    onFenye() {
      if (!this.checkMsgNull(this.bbrInfoPopList)) {
        return
      }
    ...
    checkDataForImportExcel(data) {
      if (data.length <= 0) {
        ...
        if ((data.length + this.bbrInfoPopList.length) > this.pagedata.maxEmployeesNum) {
          this.$message.error('雇员不能超过' + this.pagedata.maxEmployeesNum + '人，请删减后重新上传')
          return false
        }
        for (let i = 0; i < data.length; i++) {
          ...
          let age = GetAge(data[i]['证件编号'], this.startDate, 1)
          let day = GetDay(data[i]['证件编号'], this.startDate, 1)
          let isContentAge = false
    ...
    getSpecification() {
      ...
      this.$http.post('/prdmc/prdmcShop/productSpecification', {// 调产品方案报价接口
        ...
        if (res.data.code == 200 && res.data.msg == 'success') {
          if (this.isCache) {
            if (this.checkMsgNull(window.sessionStorage.getItem('specificationData'))) {
              this.specificationData = JSON.parse(window.sessionStorage.getItem('specificationData'))
            } else {
          ...
          this.specificationData.prdStyleSpecification.forEach((item, itemindex) => {
            item.prdDutys = (this.prdStyles)[itemindex].prdDutys
          ...
          if (this.checkMsgNull(window.sessionStorage.getItem('typeData'))) {
            this.typeData = JSON.parse(window.sessionStorage.getItem('typeData'))
          } else {
            this.typeData = this.specificationData.prdStyleSpecification[0].prdSpecificationCalcs;// 当前方案的数据明细
            window.sessionStorage.setItem('typeData', JSON.stringify(this.typeData))
          }
          if (this.checkMsgNull(window.sessionStorage.getItem('currentItemId'))) {
            this.currentItemId = JSON.parse(window.sessionStorage.getItem('currentItemId'))
          } else {
            this.currentItemId = this.specificationData.prdStyleSpecification[0].prdItemId;// 当前方案的明细id
            window.sessionStorage.setItem('currentItemId', this.currentItemId)
          }
          if (this.checkMsgNull(window.sessionStorage.getItem('currentStyleId'))) {
            this.currentStyleId = JSON.parse(window.sessionStorage.getItem('currentStyleId'))
          } else {
            this.currentStyleId = this.specificationData.prdStyleSpecification[0].styleId;// 当前方案id
            window.sessionStorage.setItem('currentStyleId', this.currentStyleId)
          }
          ...
          window.sessionStorage.setItem('specificationData', JSON.stringify(this.specificationData))
          this.getList()

        } else {
          this.$message.error('获取产品报价信息失败！')
        }
      })
    },
    generateBaseInfo() {
      this.baseInfo = []
      for (let i = 0; i < this.specificationData.prdStyleSpecification.length; i++) {
        if (this.currentStyleId == this.specificationData.prdStyleSpecification[i].styleId) {
          let calcIdsArr = this.upParams.calcIds
          for (let j = 0; j < calcIdsArr.length; j++) {
            if (this.specificationData.prdStyleSpecification[i].prdSpecificationCalcs[j].name != '总保额') {
              let entriesArr = this.specificationData.prdStyleSpecification[i].prdSpecificationCalcs[j].entries
              for (let k = 0; k < entriesArr.length; k++) {
                if (calcIdsArr[j] == entriesArr[k].id) {
                  this.baseInfo.push({
                    name: this.specificationData.prdStyleSpecification[i].prdSpecificationCalcs[j].name,
    ...
    getList() {
      ...
      this.$http.post('/prdmc/prdmcShop/insureData', {// 调获取参保项接口
        ...
        if (res.data.code == 200 && res.data.msg == 'success') {
          ...
          if (this.checkMsgNull(window.sessionStorage.getItem('enterpriseArea'))) {
          ...
          /** 阳光复工保暂时用不到营业执照 先留着 start  **/
          if (this.checkMsgNull(window.sessionStorage.getItem('businessLicenseImgUrl'))) {
          ...
          if (this.checkMsgNull(window.sessionStorage.getItem('addressCode'))) {
            this.addressCode = window.sessionStorage.getItem('addressCode')
          }

          if (this.checkMsgNull(window.sessionStorage.getItem('employees'))) {
            this.employees = window.sessionStorage.getItem('employees')
          }
          ...
          if (window.sessionStorage.getItem('startDate')) {
            ...
            this.startDefaultValue = new Date().Format('yyyy-MM-dd');// 设置起保日期默认选中日期，当前value为空时有效
          ...
          /** 计算终保日期、起保日期可选范围、终保日期可选范围 start **/
          this.startDateRange = new Date().getTime() + (this.pagedata.rangeDay * 3600 * 24 * 1000);

          if (this.pagedata.insuranceIntervalType == 0) {//固定区间：例1年
            var start_date = new Date(this.startDate);
            var end_date = new Date(this.startDate);
            if (this.pagedata.dayCount > 300) {// 如果固定区间为一年，则计算规则和其他固定区间不一样
              end_date = new Date((Number(this.startDate.split('-')[0]) + 1) + '-' + this.startDate.split('-')[1] + '-' + this.startDate.split('-')[2])
              end_date.setDate(start_date.getDate() - 1)
              end_date = end_date.Format("yyyy-MM-dd");
            } else {
              end_date.setDate(start_date.getDate() + this.pagedata.dayCount - 1)
              end_date = end_date.Format("yyyy-MM-dd");
            }
            ...
            this.pickerOptions = {
              disabledDate(time) {
                return time.getTime() < new Date().getTime() || time.getTime() > that.startDateRange;
              }
            }
            ...
            var start_date = new Date(this.startDate);
            var end_date = new Date(this.startDate);
            end_date.setDate(start_date.getDate() + this.pagedata.minDayCount - 1);
            end_date = end_date.Format("yyyy-MM-dd");
            this.endDate = end_date;
            ...
            this.pickerOptions = {
              disabledDate(time) {
                return time.getTime() < startDate || time.getTime() > that.startDateRange;
              }
            }

            this.endPickerOptions = {
              disabledDate(time) {
                return (startDate + (that.pagedata.minDayCount - 1) * 3600 * 24 * 1000) > time.getTime() || time.getTime() > ((startDate - 1) + that.pagedata.maxDayCount * 3600 * 24 * 1000);
              }
            }
          }
          /** 计算终保日期、起保日期可选范围、终保日期可选范围 end **/
          ...
          // 获取投保人、被保人、雇员信息
          this.$http.post('/sunmc/EmployerInfoQuery/EmployerByInfoParam', {
            companyId: this.companyId,
            batchIds: this.batchIds
          }).then(res => {
            if (res.data.msg == '成功' && res.data.code == 200) {
              let empInfo = res.data.data

              /** 填充投保人信息 start **/
              let pagedataCopy = JSON.parse(JSON.stringify(this.pagedata))
              for (let i = 0; i < pagedataCopy.page[0].types.length; i++) {
                let pageType = pagedataCopy.page[0].types[i].pageType
                let items = pagedataCopy.page[0].types[i].items
                if (pageType == 1) {// 投保人
                  for (let j = 0; j < items.length; j++) {
                    if (items[j].itemCode == 1) {// 姓名
                      items[j].userName = empInfo.Assured.enterpriseName
                    } else if (items[j].itemCode == 4) { // 证件号码
                      items[j].userName = empInfo.Assured.creditCode
                    } else if (items[j].itemCode == 6) { // 联系电话
                      items[j].userName = empInfo.Assured.phone
                    } else if (items[j].itemCode == 7) { // 邮箱
                      items[j].userName = empInfo.Assured.email
                    } else if (items[j].itemCode == 12) { // 所在地区
                      items[j].userCode = empInfo.Assured.agentCode
                      items[j].userName = this.getCityUserName(empInfo.Assured.agentCode)
                    } else if (items[j].itemCode == 15) { // 详细地址
                      items[j].userName = empInfo.Assured.address
                    }
                  }
                }
              }
              ...
              /** 填充雇员信息 start **/
              this.bbrInfoPopList = []
              for (let i = 0; i < empInfo.InsuranceBatch.length; i++) {
                let bbrInfoPopCopy = JSON.parse(JSON.stringify(this.bbrInfoPop))
                bbrInfoPopCopy.createdTime = new Date().getTime()
                bbrInfoPopCopy.premium = this.specificationData.price
                bbrInfoPopCopy.itemId = this.currentItemId
                bbrInfoPopCopy.jobCode = empInfo.InsuranceBatch[i].jobCode
                let jobObj = this.getJobObj(empInfo.InsuranceBatch[i].jobCode)
                bbrInfoPopCopy.jobType = jobObj.jobType
                bbrInfoPopCopy.jobTypeUserNameArr = jobObj.jobTypeUserNameArr
                for (let j = 0; j < bbrInfoPopCopy.otherOptionitems.length; j++) {
                  if (bbrInfoPopCopy.otherOptionitems[j].itemCode == 1) {// 姓名
                    bbrInfoPopCopy.otherOptionitems[j].userName = empInfo.InsuranceBatch[i].idName
                  } else if (bbrInfoPopCopy.otherOptionitems[j].itemCode == 4) {// 证件号码
                    bbrInfoPopCopy.otherOptionitems[j].userName = empInfo.InsuranceBatch[i].idIdcard
                  } else if (bbrInfoPopCopy.otherOptionitems[j].itemCode == 34) {// 职业信息
                    bbrInfoPopCopy.otherOptionitems[j].userName = jobObj.userName
                  }
                }
                this.bbrInfoPopList.push(bbrInfoPopCopy)
                this.onFenye()
              }
              /** 填充雇员信息 end **/
    ...
    getCityUserName(value) {
      for (let i = 0; i < this.citySelectOptions.length; i++) {
        let firstLabel = this.citySelectOptions[i].label
        let firstValue = this.citySelectOptions[i].value

        for (let j = 0; j < this.citySelectOptions[i].children.length; j++) {
          let secondLabel = this.citySelectOptions[i].children[j].label
          let secondValue = this.citySelectOptions[i].children[j].value
          if (this.citySelectOptions[i].children[j].value == value) {
            this.enterpriseArea = [firstValue, secondValue]
    ...
    getJobObj(value) {
      for (let i = 0; i < this.jobTypeOptions.length; i++) {
        let firstValue = this.jobTypeOptions[i].value
        for (let j = 0; j < this.jobTypeOptions[i].children.length; j++) {
          let secondValue = this.jobTypeOptions[i].children[j].value
          for (let k = 0; k < this.jobTypeOptions[i].children[j].children.length; k++) {
            let thirdValue = this.jobTypeOptions[i].children[j].children[k].value
            let thirdLabel = this.jobTypeOptions[i].children[j].children[k].label
            let jobType = this.jobTypeOptions[i].children[j].children[k].jobType
            if (thirdValue == value) {
              return {
                jobType: jobType,
                jobTypeUserNameArr: [firstValue, secondValue, thirdValue],
                userName: thirdLabel
              }
            }
          }
    ...
    changeStartTime() {
      ...
      if (this.pagedata.insuranceIntervalType == 0) { // 固定区间
        ...
        this.endDate = end_date
      } else if (this.pagedata.insuranceIntervalType == 1) { // 间隔区间
        var start_date = new Date(this.startDate);
        var end_date = new Date(this.startDate);
        end_date.setDate(start_date.getDate() + this.pagedata.minDayCount - 1);
        ...
        this.endPickerOptions = {
          disabledDate(time) {
            return (start_date.getTime() + (that.pagedata.minDayCount - 2) * 3600 * 24 * 1000) > time.getTime() || time.getTime() > (start_date.getTime() + (that.pagedata.maxDayCount - 1) * 3600 * 24 * 1000);
          }
        }
      } else {
        this.pickerOptions = {
      ...
      items.forEach(item => {
        if (itemCode == '4') {
          if (item.itemCode == '3' && item.userName == '1') { //如果是身份证的话更新价格
            ...
            console.log(this);
            var age = GetAge(userName, this.startDate, item.userName);
            var day = GetDay(userName, this.startDate, item.userName);
            console.log(age, 'age');
            console.log(day, 'day');
          ...
          for (var i = 0; i < this.entries.length; i++) {
            ...
            if (isContent(entry.value, age, day)) {
              ...
              this.$http.post("/prdmc/prdmcShop/productCalc", {
                ...
                if (res.data.code == '200') {
                  this.specificationData.price = res.data.data.price
                  this.currentItemId = res.data.data.prdItemId
                  this.specificationData.prdStyleSpecification.forEach(btnItem => {
                    if (btnItem.styleId == this.currentStyleId) {
                      btnItem.prdItemId = res.data.data.prdItemId
                      this.specificationData.prdItemId = res.data.data.prdItemId
                    }
                  })
                } else {
    ...
    getInsuredInfos() {
      for (var i = 0; i < this.pagedata.page.length; i++) {
        for (var j = 0; j < this.pagedata.page[i].types.length; j++) {
          if (this.pagedata.page[i].types[j].pageType == 2) {// 被保人信息
            this.insuredInfos = this.pagedata.page[i].types[j].items;// 被保人信息items数组
          }
        }
      }
    },
    ...
    addBeibaorenInfo() {
      if (this.bbrInfoPopList.length >= this.pagedata.maxEmployeesNum) {
        this.showBeibaoren = false
    ...
    // 用户选择方案时所调方法
    handletype(item) {
      this.specificationData.prdItemId = item.prdItemId
      ...
      this.$http.post("/prdmc/prdmcShop/productCalc", {// 产品报价测算接口
        ...
        if (res.data.code == 200) {
          this.specificationData.price = res.data.data.price;// 返回产品报价测算接口中返回的最新的价格
          this.currentItemId = res.data.data.prdItemId
          this.specificationData.prdStyleSpecification.forEach(btnItem => {
            if (btnItem.styleId == this.currentStyleId) {
              btnItem.prdItemId = res.data.data.prdItemId
          ...
          this.$http.post('/prdmc/prdmcShop/insureData', {
            ...
            if (res.data.code == '200') {
              this.pagedata.maxDayCount = res.data.data.maxDayCount
              this.pagedata.minDayCount = res.data.data.minDayCount
              this.pagedata.dayCount = res.data.data.dayCount
              ...
              if (this.bbrInfoPopList.length > 0) {
                for (let i = 0; i < this.bbrInfoPopList.length; i++) {
                  ...
                  //   }
                  // })
                  this.bbrInfoPopList[i].premium = this.specificationData.price
                  this.bbrInfoPopList[i].itemId = this.currentItemId
                }
                window.sessionStorage.setItem('bbrInfoPopList', JSON.stringify(this.bbrInfoPopList))
              }
            } else {
    ...
    handleClick(item) {
      // 将所有方案中相同的项目都改成一致
      let btnlistCopy = JSON.parse(JSON.stringify(this.specificationData.prdStyleSpecification))
      ...
      this.specificationData.prdStyleSpecification = JSON.parse(JSON.stringify(btnlistCopy))
    ...
    gotoConfirm() {
      ...
      if (this.bbrInfoPopList.length < this.pagedata.minEmployeesNum) {
        this.$message.error("雇员最少为" + this.pagedata.minEmployeesNum + '人')
        return
      }
      if (this.bbrInfoPopList.length > this.pagedata.maxEmployeesNum) {
        this.$message.error("雇员最多为" + this.pagedata.maxEmployeesNum + '人')
        return
      }
      ...
      window.sessionStorage.removeItem('specificationData')
      ...
      window.sessionStorage.removeItem('addressCode')
...
// 将 P920091615004924408 改造成 T920091615004924408 ，然后跟轨迹id进行绑定
    ...
    handleCityChange(i, myCityArr) {
      window.sessionStorage.setItem('enterpriseArea', JSON.stringify(myCityArr))
      let fullName = ''
  ...
  created() {
    ...
    if (this.checkMsgNull(prdId) && this.prdId == prdId) {
      this.isCache = true
    } else {
      window.sessionStorage.setItem('prdId', this.prdId)
    }
