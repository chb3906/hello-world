  ...
  <div class="Pro_insur">
    ...
    <div>
      ...
      <div class="insur" v-for='(page,i) in pagedata.page' :key="i+'pageType1'" v-if='i+1 == num'>
        <div class="policyholderinfo" v-for="(info,index) in page.types" :key="index+'pageitem'" v-if="info.pageType == 1">
          <h1>{{info.pageType | statusFilter}}</h1>
          <div class="insur_recognizee">
            <div class="itemDiv" v-for="(i) in info.items" :key='i.itemName' v-if="i.isShow && i.itemCode != 17">
              ...
              <!--投保人姓名-->
              <input type="text" placeholder="请输入本人姓名" v-if="i.itemCode == 1" class="itemInput" v-model="i.userName" @blur="handleCommentFocus(i,info.pageType,groupRiskFlag),handlePagedataToSession()" @click="onSensors('填单页-填写投保人姓名', $event)" @change="getSlAndDiscount" :maxlength="30">
              <!--证件类型-->
              ...
              <!--证件号码-->
              <input type="text" placeholder="请输入本人证件号码" v-else-if="i.itemCode == 4" class="itemInput" v-model="i.userName" maxlength="18" @change='onTbrIDNoOrAge(i.itemCode,i.userName),getSlAndDiscount()' @keyup="getSlAndDiscount(),onTbrIDKeyup(i.userName)" @blur="handleCommentFocus(i,info.pageType,groupRiskFlag),handlePagedataToSession()" @click="onSensors('填单页-填写投保人证件号', $event)">
              <!--手机号-->
              ...
              <!--车牌号-->
              <input type="text" :placeholder="isNewCar ? '新车':'例 京AF5566'" v-else-if="!i.isHasOption && i.itemCode == 36" class="itemInput" v-model="i.userName" @blur="handleCommentFocus(i,info.pageType,groupRiskFlag),handlePagedataToSession()" @click="onSensors('填单页-填写车牌号', $event)" maxlength="8" :disabled="isNewCar" @keyup="handleCommonTypeIn(i)">
              <!--车架号-->
              ...
              <!-- 反洗钱出单传值开关 -->
              <select v-show="false" v-else-if="i.itemCode == 87" class="itemSelect" v-model="i.userName">
                <option v-for="itemy in i.options" :value="itemy.id" :key="itemy.id">{{itemy.name}}</option>
              </select>
            ...
            <div class="itemDiv" v-if="sheBaoEntries.length > 0 && groupRiskFlag == 1">
              ...
              <div class="shebao-wrap">
                <div class="shebao-item" v-for="(item, index) in sheBaoEntries" :key="index" :style="{backgroundColor: (index == tbrCalcInfo.sheBaoIndex ? prdTheme: '#fff'),color: (index == tbrCalcInfo.sheBaoIndex ? '#fff': '#B2B2B2'),border: (index == tbrCalcInfo.sheBaoIndex ? '1px solid ' + prdTheme : '1px solid #B2B2B2')}" @click="onTbrSheBaoSelect(item.id, index),onSensors('填单页-选择投保人社保情况', $event)">{{item.value}}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="insur_recognizee">
            ...
            <div v-if='groupRiskFlag == 1'>
              <div class="duoBbr-wrap">
                <div class="duoBbr-item" v-for="(bbrInfo, bbrIndex) in bbrInfoPopList" :key="bbrIndex + 'bbr'">
                  <template v-if="bbrInfo.userName == 1">
                    <div class="duoBbr-self">
                      <div class="duoBbr-self-left">
                        <div class="duoBbr-self-name">本人</div>
                        <template v-if="sheBaoEntries.length > 0 && tbrCalcInfo.sheBaoIndex == idx" v-for="(item,idx) in sheBaoEntries">
                          <div class="benren-relation">{{item.value}}</div>
                        </template>
                      </div>
                      <div class="duoBbr-icon" @click="deleteBbrInfo(bbrIndex)"><img src="../../../static/images/icon_delete_btn.png" alt=""></div>
                    </div>
                  </template>
                  <template v-else>
                    <div class="duoBbr-other">
                      <div class="duoBbr-other-left">
                        ...
                        <template v-if="sheBaoEntries.length > 0 && bbrInfo.sheBaoIndex == idx" v-for="(item,idx) in sheBaoEntries">
                          <div class="duoBbr-relation">{{item.value}}</div>
                        </template>
                      </div>
                      <div class="duoBbr-other-right">
                        <div class="duoBbr-icon" @click="updateBbrInfo(bbrIndex)"><img src="../../../static/images/icon_change_btn.png" alt=""></div>
                        <div class="duoBbr-icon" @click="deleteBbrInfo(bbrIndex)"><img src="../../../static/images/icon_delete_btn.png" alt=""></div>
                      </div>
                    </div>
              ...
              <div class="insur_recognizee">
                <div class="addBbrInfo" v-show="bbrInfoPopList.length < pagedata.maxGroupNum" @click="onShowMoreBbr" :style="{color: prdTheme}">+ 添加被保险人</div>
              </div>
            </div>
            <!--被保人->个险-->
            <div v-else>
              ...
              <template v-for="(wWai, indexWai) in info.items" v-if="wWai.itemCode == 8 && wWai.userName != 1">
                <div class="itemDiv" v-for="(w,index) in info.items" :key='w.itemName' v-if="w.itemCode != 8 && w.isShow">
                  ...
                  <!-- 详细地址 -->
                  <input type="text" placeholder="请输入详细地址" v-else-if="!w.isHasOption && w.itemCode == 15" class="itemInput" v-model="w.userName" @blur="handleCommentFocus(w,info.pageType,groupRiskFlag),handlePagedataToSession()" @click="onSensors('填单页-填写被保人详细地址', $event)" maxlength="30">
                  <!-- 财产所在地地址 -->
                  <input type="text" placeholder="请输入财产所在地地址" v-else-if="!w.isHasOption && w.itemCode == 25" class="itemInput" v-model="w.userName" @blur="handleCommentFocus(w,info.pageType,groupRiskFlag),handlePagedataToSession()" @click="onSensors('填单页-填写被保人财产所在地地址', $event)" maxlength="30">
        ...
        <div class="policyholderinfo" v-for="(info,index) in page.types" :key="index+'pageitem'" v-if="info.pageType == 7">
          ...
          <div class="insur_recognizee">
            <div class="itemDiv" v-for="(i) in info.items" :key='i.itemName' v-if="singleTargetFlag && i.itemCode != 69">
              ...
              <!-- 车牌号 -->
              <input type="text" :placeholder="isNewCar ? '新车':'例 京AF5566'" v-else-if="i.itemCode == 36" class="itemInput" v-model="i.userName" @blur="handleCheckTarget(i),handlePagedataToSession()" maxlength="8" :disabled="isNewCar" @keyup="handleCommonTypeIn(i)" @click="onSensors('填单页-填写车牌号', $event)">
              <!-- 营业面积 -->
        ...
        <div class="moreBbr-content">
          <div class="group-item-insur">
            ...
            <div class="itemDiv" v-for="(w,indexx) in bbrInfoPop.otherOptionitems" :key="indexx+'group-items-info'" v-if="w.isShow && w.itemCode != 8">
              ...
              <!-- 性别 -->
              <select :key="indexx+'group--index'" v-else-if="w.isHasOption &&  w.itemCode == 2" class="itemSelect" v-model="w.userName" @change="onSensors('填单页-选择被保人性别', $event)">
                <option v-for="itemy in w.options" :value="itemy.id" :key="itemy.id">{{itemy.name}}</option>
              </select>
              <!-- 出生日期 -->
            ...
            <div class="itemDiv" v-if="sheBaoEntries.length > 0 && bbrInfoPop.userName != 1">
              <span class="itemSpan">社保情况</span>
              <div class="shebao-wrap">
                <div class="shebao-item" v-for="(item, index) in sheBaoEntries" :key="index" :style="{backgroundColor: (index == bbrInfoPop.sheBaoIndex ? prdTheme: '#fff'),color: (index == bbrInfoPop.sheBaoIndex ? '#fff': '#B2B2B2'),border: (index == bbrInfoPop.sheBaoIndex ? '1px solid ' + prdTheme : '1px solid #B2B2B2')}" @click="onBbrSheBaoSelect(item.id, index),onSensors('填单页-选择被保人社保情况', $event)">{{item.value}}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="bbrPriceDetail" v-if="showBbrPriceDetail">
        ...
        <div class="bbrPriceDetail-body">
          <div class="bbrPriceDetail-body-item" v-for="(item, index) of bbrInfoPopList" :key="index">
            ...
            <div class="bbrPriceDetail-body-item-value">￥{{item.price}}</div>
          </div>
        </div>
      </div>
    </transition>
    ...
    <div class="proInsur-foot">
      ...
      <div class="foot-wrap">
        ...
        <div class="ljzf" @click="topay(),onSensors('填单页-点击立即支付', $event)" v-else-if="pagedata.giveInsurance == 0" :style="{backgroundColor: prdTheme}">免费领取</div>
      </div>
    </div>
    <!--弹框提示-->
...
export default {
  ...
  data() {
    return {
      ...
      fabbrsEntries: JSON.parse(window.sessionStorage.getItem('fabbrsEntries')),
      ...
      innerDiscount: window.sessionStorage.getItem("innerDiscount"),// 内部折扣
  ...
  created() {
    ...
    console.log(prdId + ' ' + this.$route.query.prdId)
    console.log(styleId + ' ' + this.$route.query.styleId)
    if (
      this.checkMsgNull(prdId) &&
      this.checkMsgNull(styleId) &&
      this.$route.query.prdId == prdId &&
      this.$route.query.styleId == styleId
    ) {
      this.isCache = true
      console.log('走缓存。。。')
    } else {
  ...
  methods: {
    ...
    getSlAndDiscount() {
      if (!this.hasItemCode(1, 17)) {// 如果投保人中有司龄这个字段，才去调接口
        return false
      }
      let name = this.getItemUserName(1, 1)
      let idCardNo = this.getItemUserName(1, 4)
      if (!this.common.checkMsgNull(name) || !this.common.checkMsgNull(idCardNo)) {
        return false
      }
      if (!this.common.checkName(name) || !this.common.checkIdCard(idCardNo)) {
        return false
      }
      this.$http.post('/sunmc/sunSigBusiness/selectSigBusinessNationalVw', {
        idCardCode: idCardNo,
        idCardName: name,
        ...
        if (res.data.code == 200) {
          let companyAge = res.data.data.companyAge
          let activityDiscount = res.data.data.activityDiscount
          if (this.common.checkMsgNull(companyAge) && this.common.checkMsgNull(activityDiscount)) {
            this.setSingleUserName(1, '17', companyAge)
            this.setShowFlag(1, ['17'], true)
            this.innerDiscount = activityDiscount
            window.sessionStorage.setItem('innerDiscount', this.innerDiscount)
          } else {
            this.setSingleUserName(1, '17', '')
            this.setShowFlag(1, ['17'], false)
            this.innerDiscount = ''
            window.sessionStorage.removeItem('innerDiscount')
          }
          this.onSetMoneyShow()
        }
      })
    ...
    onTbrSheBaoSelect(id, index) {
      this.tbrCalcInfo.sheBaoId = id
      this.tbrCalcInfo.sheBaoIndex = index

      /* 处理报价因子：社保情况的id  开始 */
      if (this.sheBaoEntries.length > 0 && this.groupRiskFlag == 1) {
        for (let h = 0; h < this.tbrCalcInfo.calcIdKeys.length; h++) {
          if (this.tbrCalcInfo.calcIdKeys[h] == '社保情况') {
            this.tbrCalcInfo.calcIds[h] = this.tbrCalcInfo.sheBaoId
          }
        }
      }
      /* 处理报价因子：社保情况的id  结束 */
      this.onProductCalc({
        styleId: this.tbrCalcInfo.styleId,
        calcIds: this.tbrCalcInfo.calcIds
      }, res => {
        if (res.data.code == '200') {
          this.tbrCalcInfo.price = res.data.data.price // 投保人保费
          this.tbrCalcInfo.prdItemId = res.data.data.prdItemId // 投保人itemId
          window.sessionStorage.setItem('tbrCalcInfo', JSON.stringify(this.tbrCalcInfo))
          if (this.groupRiskFlag == 1) {// 如果是（多被保人）
            if (this.bbrInfoPopList.length > 0) {
              for (let i = 0; i < this.bbrInfoPopList.length; i++) {
                if (this.bbrInfoPopList[i].userName == 1) {// 如果被保人数组中有本人，则把当前计算出的保费给本人
                  this.bbrInfoPopList[i].price = res.data.data.price
                  this.bbrInfoPopList[i].prdItemId = res.data.data.prdItemId
                  this.bbrInfoPopList[i].sheBaoId = this.tbrCalcInfo.sheBaoId
                  this.bbrInfoPopList[i].sheBaoIndex = this.tbrCalcInfo.sheBaoIndex
                  this.bbrInfoPopList[i].calcIds = this.tbrCalcInfo.calcIds
                }
              }
              window.sessionStorage.setItem('bbrInfoPopList', JSON.stringify(this.bbrInfoPopList))
            }
          } else {
            if (this.checkItemValue(2, 8, ['1'])) {
              this.danBbrCalcInfo.price = res.data.data.price
              this.danBbrCalcInfo.prdItemId = res.data.data.prdItemId
              this.danBbrCalcInfo.calcIds = this.tbrCalcInfo.calcIds
            }
          }
          this.onSetMoneyShow()
        }
    ...
    onBbrSheBaoSelect(id, index) {
      this.bbrInfoPop.sheBaoId = id
      this.bbrInfoPop.sheBaoIndex = index
    },
    ...
    deleteBbrInfo(index) {
      ...
      if (this.bbrInfoPopList.length >= 2 && this.fabbrsEntries.length > 0) {
      ...
      if (this.updateBbrInfoPopFlag) {// 如果是更新状态
        ...
        if (this.bbrInfoPop.userName == 1) {// 如果是本人，则判断投保人信息是否已经填写，若没有填写，提醒填写；若已经填写，则正常往下走
          if (!this.checkTbrInfo()) {
            ...
            this.bbrInfoPop.price = this.tbrCalcInfo.price
            this.bbrInfoPop.prdItemId = this.tbrCalcInfo.prdItemId
            this.bbrInfoPop.calcIds = this.tbrCalcInfo.calcIds
            if (this.sheBaoEntries.length > 0) {
              this.bbrInfoPop.sheBaoId = this.tbrCalcInfo.sheBaoId
              this.bbrInfoPop.sheBaoIndex = this.tbrCalcInfo.sheBaoIndex
            }
            this.bbrInfoPopList.push(JSON.parse(JSON.stringify(this.bbrInfoPop)))
            if (this.bbrInfoPopList.length >= 3 && this.fabbrsEntries.length > 0) {
              this.handleFabbrsChange()
            } else {
              window.sessionStorage.setItem('bbrInfoPopList', JSON.stringify(this.bbrInfoPopList))
              this.onSetMoneyShow()
    ...
    handleFabbrsChange() {
      let id = ''
      let index = ''
      let fabbrsEntries = JSON.parse(window.sessionStorage.getItem('fabbrsEntries'))
      let upParams = JSON.parse(window.sessionStorage.getItem('upParams'))
      for (let i = 0; i < upParams.calcIdKeys.length; i++) {
        if (upParams.calcIdKeys[i] == '方案被保人数') {
          index = i
          for (let j = 0; j < fabbrsEntries.length; j++) {
            if (fabbrsEntries[j].value == this.bbrInfoPopList.length) {
              id = fabbrsEntries[j].id
            }
          }
        }
      }

      upParams.calcIds[index] = id
      window.sessionStorage.setItem('upParams', JSON.stringify(upParams))

      this.tbrCalcInfo.calcIds[index] = id
      for (let k = 0; k < this.bbrInfoPopList.length; k++) {
        this.bbrInfoPopList[k].calcIds[index] = id
      }
      ...
      this.onProductCalc({
        ...
        this.tbrCalcInfo.price = res.data.data.price
        this.tbrCalcInfo.prdItemId = res.data.data.prdItemId
        window.sessionStorage.setItem('tbrCalcInfo', JSON.stringify(this.tbrCalcInfo))
        this.handleList(0)
      })
    ...
    handleList(i) {
      this.onProductCalc({
        styleId: this.bbrInfoPopList[i].styleId,
        calcIds: this.bbrInfoPopList[i].calcIds
      }, res => {
        if (res.data.code == '200') {
          this.bbrInfoPopList[i].prdItemId = res.data.data.prdItemId
          this.bbrInfoPopList[i].price = res.data.data.price
    ...
    // 调报价测算接口
    onProductCalc(obj, callback) {
      this.$http.post('/prdmc/prdmcShop/productCalc', obj).then(res => {
        callback(res)
      }).catch((e) => console.log(e))
    },
    handleTbrSex(userName) {
      ...
      if (sexEntries && sexEntries.length > 0) {// 方案中有性别区间
        for (let y = 0; y < this.tbrCalcInfo.calcIdKeys.length; y++) {
          if (this.tbrCalcInfo.calcIdKeys[y] == '性别') {// 如果方案字段中有性别选项，则需要根据身份证号判断性别
            if (this.checkMsgNull(this.getSexId(sexName))) {
              this.tbrCalcInfo.calcIds[y] = this.getSexId(sexName)
            } else {
              if (this.checkItemValue(2, 8, ['1'])) {
              ...
              if (entries && entries.length > 0) {
                ...
                for (let j = 0; j < entries.length; j++) {
                  if (isContent(entries[j].value, age, day)) {
                    isContentAge = true
                    this.tbrCalcInfo.calcIds[getAgeIndex(this.tbrCalcInfo)] = entries[j].id
                  }
                }
                if (!isContentAge && this.checkItemValue(2, 8, ['1'])) {
    ...
    onDuoBbrIDNoOrAge(callback) {
      ...
      if (this.sheBaoEntries.length > 0 && this.groupRiskFlag == 1) {
        for (let h = 0; h < upParams.calcIdKeys.length; h++) {
          if (upParams.calcIdKeys[h] == '社保情况') {
            upParams.calcIds[h] = this.bbrInfoPop.sheBaoId
          }
        }
      }
      ...
      this.onProductCalc({
        ...
        calcIds: upParams.calcIds
      }, res => {
        if (res.data.code == '200') {
          this.bbrInfoPop.price = res.data.data.price
          this.bbrInfoPop.prdItemId = res.data.data.prdItemId
          this.bbrInfoPop.calcIds = upParams.calcIds
          callback();
        } else {
    ...
    onDanBbrIDNoOrAge(itemCode, userName) {
            ...
            if (!this.checkIdCard(userName)) {// 身份证号校验未通过
              ...
              if (entries && entries.length > 0) {
                ...
                for (let j = 0; j < entries.length; j++) {
                  if (isContent(entries[j].value, age, day)) {
                    isContentAge = true
                    this.danBbrCalcInfo.calcIds[getAgeIndex(this.danBbrCalcInfo)] = entries[j].id
                  }
                }
                if (!isContentAge) {
      ...
      this.onProductCalc({
        styleId: this.danBbrCalcInfo.styleId,
        calcIds: this.danBbrCalcInfo.calcIds
      }, res => {
        if (res.data.code == '200') {
          this.danBbrCalcInfo.price = res.data.data.price
          this.danBbrCalcInfo.prdItemId = res.data.data.prdItemId
    ...
    onSetMoneyShow() {
      ...
      if (this.groupRiskFlag == 0) {// 单被保人
        if (this.checkItemValue(2, 8, ['1'])) {
          handlePrice = this.tbrCalcInfo.price
        } else {
          handlePrice = this.danBbrCalcInfo.price
        }
      } else if (this.groupRiskFlag == 1) { //多被保人
        if (this.bbrInfoPopList.length == 0) {
          handlePrice = this.tbrCalcInfo.price
        } else if (this.bbrInfoPopList.length > 0) {
          if (this.pagedata.isSharedPremium == 0) {// 共享保费
            handlePrice = this.bbrInfoPopList[0].price
          } else if (this.pagedata.isSharedPremium == 1) {// 不共享保费
            for (let i = 0; i < this.bbrInfoPopList.length; i++) {
              handlePrice += Number(this.bbrInfoPopList[i].price)
            }
          }
        }
      }
      ...
      let currentDiscount = ''
      if (this.common.checkMsgNull(this.discount)) {
        if (this.common.checkMsgNull(this.innerDiscount)) {
          currentDiscount = this.innerDiscount
        } else {
          currentDiscount = this.discount
        }
      }

      if (this.common.checkMsgNull(currentDiscount)) {//若有折扣
        this.money = this.common.accMul(handlePrice, currentDiscount).toFixed(2)
        discountPrice = this.common.accMul(handlePrice, currentDiscount).toFixed(2) //价格*折扣
      }
    ...
    handleTbrInfo(callback) {
            ...
            if (entries && entries.length > 0) {//有年龄区间
              ...
              for (let i = 0; i < entries.length; i++) {
                if (isContent(entries[i].value, age, day)) {//true
                  this.tbrCalcInfo.calcIds[getAgeIndex(this.tbrCalcInfo)] = entries[i].id
                  isContentAge = true
                }
              }
              if (!isContentAge && this.checkItemValue(2, 8, ['1'])) {
      ...
      if (fabbrsEntries && fabbrsEntries.length > 0) {
        if (this.bbrInfoPopList.length >= 3) {// 方案被保人数  大于等于3的时候，才需要修改该因子的id
          for (let h = 0; h < this.tbrCalcInfo.calcIdKeys.length; h++) {
            if (this.tbrCalcInfo.calcIdKeys[h] == '方案被保人数') {
              for (let w = 0; w < fabbrsEntries.length; w++) {
                if (fabbrsEntries[w].value == this.bbrInfoPopList.length) {
                  this.tbrCalcInfo.calcIds[h] = fabbrsEntries[w].id
                }
              }
            }
          }
    ...
    handleDuoBbrInfo(i) {
      ...
      if (item.userName == 1) {// 本人
        this.bbrInfoPopList[i].prdItemId = this.tbrCalcInfo.prdItemId
        this.bbrInfoPopList[i].price = this.tbrCalcInfo.price
        this.bbrInfoPopList[i].styleId = this.tbrCalcInfo.styleId
        this.bbrInfoPopList[i].calcIds = this.tbrCalcInfo.calcIds
        this.bbrInfoPopList[i].calcIdKeys = this.tbrCalcInfo.calcIdKeys
        i++
        if (i < this.bbrInfoPopList.length) {
        ...
        if (cardType == 1) {
          ...
          if (entries && entries.length > 0) {//有年龄区间
            for (let i = 0; i < entries.length; i++) {
              if (isContent(entries[i].value, age, day)) {//true
                item.calcIds[getAgeIndex(item)] = entries[i].id
              }
            }
          }
        } else if (cardType == 2) {
        ...
        if (sheBaoEntries && sheBaoEntries.length > 0) {
          for (let h = 0; h < item.calcIdKeys.length; h++) {
            if (item.calcIdKeys[h] == '社保情况') {
              item.calcIds[h] = item.sheBaoId
            }
          }
        }
        if (fabbrsEntries && fabbrsEntries.length > 0) {
          if (this.bbrInfoPopList.length >= 3) {// 方案被保人数  大于等于3的时候，才需要修改该因子的id
            for (let h = 0; h < item.calcIdKeys.length; h++) {
              if (item.calcIdKeys[h] == '方案被保人数') {
                for (let w = 0; w < fabbrsEntries.length; w++) {
                  if (fabbrsEntries[w].value == this.bbrInfoPopList.length) {
                    item.calcIds[h] = fabbrsEntries[w].id
                  }
                }
              }
            }
    ...
    getListNew() {
      ...
      this.$http.post("/prdmc/prdmcShop/insureData", {
        ...
        if (this.isCache) {
          ...
          // 改变报价因子组合
          if (this.checkMsgNull(window.sessionStorage.getItem('tbrCalcInfo'))) {// 投保人报价信息
            this.tbrCalcInfo = JSON.parse(window.sessionStorage.getItem('tbrCalcInfo'))
            let upParams = JSON.parse(window.sessionStorage.getItem('upParams'))
            this.tbrCalcInfo.calcIds = upParams.calcIds
            for (let i = 0; i < upParams.calcIdKeys.length; i++) {
              if (upParams.calcIdKeys[i] == '社保情况') {
                for (let j = 0; j < this.sheBaoEntries.length; j++) {
                  if (upParams.calcIds[i] == this.sheBaoEntries[j].id) {
                    this.tbrCalcInfo.sheBaoIndex = j
                    this.tbrCalcInfo.sheBaoId = this.sheBaoEntries[j].id
                    this.tbrCalcInfo.calcIds[i] = this.sheBaoEntries[j].id
                  }
                }
              }
            }
          ...
          if (this.groupRiskFlag == 1) {//多个被保人
            ...
            if (this.checkMsgNull(window.sessionStorage.getItem('bbrInfoPopList'))) {
              ...
              let upParams = JSON.parse(window.sessionStorage.getItem('upParams'))
              for (let i = 0; i < this.bbrInfoPopList.length; i++) {
                this.bbrInfoPopList[i].calcIds = upParams.calcIds
                if (this.bbrInfoPopList[i].userName == 1) {// 如果被保人中有本人，需要改变一下本人的社保情况
                  if (this.bbrInfoPopList[i].calcIdKeys[j] == '社保情况') {
                    this.bbrInfoPopList[i].calcIds[j] = this.tbrCalcInfo.sheBaoId
                  }
                } else {
                  for (let j = 0; j < this.bbrInfoPopList[i].calcIdKeys.length; j++) {
                    if (this.bbrInfoPopList[i].calcIdKeys[j] == '社保情况') {
                      this.bbrInfoPopList[i].calcIds[j] = this.bbrInfoPopList[i].sheBaoId
                    }
                  }
                }
              }
              // 改变upParams 报价因子“方案被保人数”的id
              let fabbrsEntries = JSON.parse(window.sessionStorage.getItem('fabbrsEntries'))
              for (let h = 0; h < fabbrsEntries.length; h++) {
                if (fabbrsEntries[h].value == this.bbrInfoPopList.length) {
                  let id = fabbrsEntries[h].id
                  for (let k = 0; k < upParams.calcIdKeys.length; k++) {
                    if (upParams.calcIdKeys[k] == '方案被保人数') {
                      upParams.calcIds[k] = id
                      window.sessionStorage.setItem('upParams', JSON.stringify(upParams))
                      break
                    }
                  }
          ...
          if (this.groupRiskFlag == 1) {//多个被保人
            let upParams = JSON.parse(window.sessionStorage.getItem('upParams'))
            this.bbrInfoPop = {}
            this.bbrInfoPop.price = upParams.price // 设置多被保人的初始保费为0
            this.bbrInfoPop.prdItemId = upParams.prdItemId // 设置itemId 方案报价id为空
            this.bbrInfoPop.styleId = upParams.styleId
            this.bbrInfoPop.calcIds = upParams.calcIds
            this.bbrInfoPop.calcIdKeys = upParams.calcIdKeys
            if (this.sheBaoEntries.length > 0) {
              for (let i = 0; i < this.bbrInfoPop.calcIdKeys.length; i++) {
                if (this.bbrInfoPop.calcIdKeys[i] == '社保情况') {
                  this.bbrInfoPop.sheBaoId = this.bbrInfoPop.calcIds[i]
                }
              }
              for (let j = 0; j < this.sheBaoEntries.length; j++) {
                if (this.sheBaoEntries[j].id == this.bbrInfoPop.sheBaoId) {
                  this.bbrInfoPop.sheBaoIndex = j
                }
              }
            }
    ...
    getTbrCalcInfo() {
      let upParamsCopy = JSON.parse(window.sessionStorage.getItem('upParams'))
      this.tbrCalcInfo.price = upParamsCopy.price
      this.tbrCalcInfo.prdItemId = upParamsCopy.prdItemId
      this.tbrCalcInfo.styleId = upParamsCopy.styleId
      this.tbrCalcInfo.calcIds = upParamsCopy.calcIds
      this.tbrCalcInfo.calcIdKeys = upParamsCopy.calcIdKeys
      if (this.sheBaoEntries.length > 0) {
        for (let i = 0; i < this.tbrCalcInfo.calcIdKeys.length; i++) {
          if (this.tbrCalcInfo.calcIdKeys[i] == '社保情况') {
            this.tbrCalcInfo.sheBaoId = this.tbrCalcInfo.calcIds[i]
          }
        }
        for (let j = 0; j < this.sheBaoEntries.length; j++) {
          if (this.sheBaoEntries[j].id == this.tbrCalcInfo.sheBaoId) {
            this.tbrCalcInfo.sheBaoIndex = j
          }
        }
      }
    ...
    getDanBbrCalcInfo() {
      let upParamsCopy = JSON.parse(window.sessionStorage.getItem('upParams'))
      this.danBbrCalcInfo.price = upParamsCopy.price
      this.danBbrCalcInfo.prdItemId = upParamsCopy.prdItemId
      this.danBbrCalcInfo.styleId = upParamsCopy.styleId
      this.danBbrCalcInfo.calcIds = upParamsCopy.calcIds
      this.danBbrCalcInfo.calcIdKeys = upParamsCopy.calcIdKeys
    },
    // 给有options选项的添加默认值
    setUserName() {
      for (let i = 0; i < this.pagedata.page[0].types.length; i++) {
    ...
    // 单被保人根据报价因子变化重新计算保费
    onDanBbrCalc() {
      if (this.checkItemValue(2, 8, ['1'])) {// 如果被保人当前是本人，走这块儿
        this.danBbrCalcInfo.prdItemId = this.tbrCalcInfo.prdItemId
        this.danBbrCalcInfo.price = this.tbrCalcInfo.price
        this.danBbrCalcInfo.calcIds = this.tbrCalcInfo.calcIds
        this.onSetMoneyShow()
      } else { // 如果单被保人关系当前不是本人，走这块儿
        let checkDanBbrFlag = true
        let entries = JSON.parse(window.sessionStorage.getItem("entries"));
        let sexEntries = JSON.parse(window.sessionStorage.getItem("sexEntries"))
            ...
            if (!this.checkIdCard(idCardNumber)) {// 身份证号校验未通过
              ...
              if (entries && entries.length > 0) {//有年龄区间
                ...
                for (var i = 0; i < entries.length; i++) {
                  if (isContent(entries[i].value, age, day)) {//true
                    this.danBbrCalcInfo.calcIds[getAgeIndex(this.danBbrCalcInfo)] = entries[i].id
                    checkDanBbrFlag = true
                    isContentAge = true
                  }
                }
                if (!isContentAge) {
              ...
              if (sexEntries && sexEntries.length > 0) {// 方案中有性别区间
                for (let y = 0; y < this.danBbrCalcInfo.calcIdKeys.length; y++) {
                  if (this.danBbrCalcInfo.calcIdKeys[y] == '性别') {// 如果方案字段中有性别选项，则需要根据身份证号判断性别
                    let len = idCardNumber.length
                    let sexName = ''
                    if (len == 15) {
                      if (parseInt(idCardNumber.substr(14, 1)) % 2 == 1) {// 男
                    ...
                    if (this.checkMsgNull(this.getSexId(sexName))) {
                      this.danBbrCalcInfo.calcIds[y] = this.getSexId(sexName)
                    } else {
    ...
    handleCheckTarget(item) {
      ...
      } else if (item.itemCode == 35 || item.itemCode == 94) { // 车架号
        if (this.isNewCar) {// 如果是新车
            ...
            if (this.groupRiskFlag == 1) { //多个被保人
              for (var i = 0; i < this.bbrInfoPopList.length; i++) {
                var obj = {
                  ...
                  itemId: this.bbrInfoPopList[i].prdItemId,
                  insuredItems: []
                }
              ...
              var obj = {
                linkmanId: this.linkmanId,
                itemId: this.danBbrCalcInfo.prdItemId,
                insuredItems: []
              }
              if (this.checkItemValue(2, 8, ['1'])) {//若是本人
                obj.itemId = this.tbrCalcInfo.prdItemId
                obj.insuredItems.push({
                  code: '8',
                  value: '1'
                })
      ...
      for (var m = 0; m < this.pagedata.page[0].types.length; m++) {
        ...
        if (z.pageType == 1) {//投保人
          ...
          if (this.groupRiskFlag == 1) {//多个被保人
            ...
            if (!this.checkItemValue(2, 8, ['1'])) { // 如果被保人不是本人
              for (let i = 0; i < this.pagedata.page[0].types.length; i++) {
                let type = this.pagedata.page[0].types[i]
                if (type.pageType == 2) {
                  for (let j = 0; j < type.items.length; j++) {
    ...
    topay() {
      ...
      window.sessionStorage.setItem('innerDiscount', this.innerDiscount);//内部折扣
